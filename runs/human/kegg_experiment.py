...
from preprocessing.human.consts import GENERATED_HUMAN_NET_PATH, GENERATED_KEGG_ANNOTATIONS_PATH, GENERATED_EXPERIMENT_PATH, \
...
if __name__ == '__main__':
    ...
    feature_cols, reverse_cols = get_features(network, experiments,
                                              output_directory=HUMAN_RESULTS_DIR, force=True, save=True)
    ...
    false_annotations = set(annotated_edges).intersection(opposite_edges)
    ...
    print(f"Not annotated: {unannotated_num} out of {len(true_annotations)}")
    ...
    ax.set_title('Single experiment D2D scores distribution for pathway hsa00980')
    plt.savefig(r'C:\git\BNet21-D2D\results\single_unified_experiment\SingleExperimentD2DscoresDistribution.pdf',
                format='pdf')
    plt.show()
